...
public class ForwardIFCAnalysisProblem extends AbstractIFCAnalysisProblem {
    ...
    public ForwardIFCAnalysisProblem(CGNode node, IFCAnalysisFactDomain domain, ISupergraph<BasicBlockInContext<IExplodedBasicBlock>, CGNode> supergraph) {
        super(node, domain, supergraph);
    }
    ...
    public Collection<PathEdge<BasicBlockInContext<IExplodedBasicBlock>>> initialSeeds() {
        ...
        for (int i = 0; i < symbolTable.getMaxValueNumber(); ++i) {
            int factId = this.domain.add(new IfcAnalysisFact(this.entrypoint, symbolTable.getIntValue(i), null, IFCLabel.PUBLIC));
